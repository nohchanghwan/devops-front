<template>
  <v-app-bar class="main-header" height="48" fixed color='primary' elevation="1" dark>
    ...
    <v-toolbar-title>KB Contents Management System</v-toolbar-title>
    <v-spacer></v-spacer>
    <Search />
    ...
    <v-menu min-width="180" offset-y bottom left nudge-bottom="10">
      ...
      <v-list>
        <div class="text-h6 grey--text text--darken-3 px-4 pt-4">{{showUserInfo}}</div>
        <v-list-item-group color="primary">
        ...
        <div class="d-flex justify-center my-3">
          <v-btn width="80%" large outlined color="primary" class="text-capitalize" @click="logOut" v-if="showUserInfo">
            로그아웃
          </v-btn>
          <v-btn width="80%" large outlined color="primary" class="text-capitalize" @click='logIn' v-else>
            로그인
          </v-btn>
        </div>
      </v-list>
